...
while ~done
    ...
    if plotOn
        subplot(121), imshow(uint8(y)),axis image, title(['iteration times = ' num2str(iter) ]);
        subplot(122), plot(0:iter, MS ), xlabel('iteration #'), ylabel('averaged mean shift');axis square; title(['; averaged mean-shift = ' num2str(yMS)])
        drawnow
    end
